...
class EventSlicer:
    def __init__(self, h5f: h5py.File):
        ...
        if "t_offset" in list(h5f.keys()):
            self.t_offset = np.float64(h5f['t_offset'][()])
        else:
            self.t_offset = np.float64(0)
        # print(self.events['t'][0], self.events['t'][-1])
    ...
    def get_events(self, t_start_us: np.float64, t_end_us: np.float64, max_events_per_data: int = -1) -> Dict[str, np.ndarray]:
        ...
        # print(idx_start_offset, idx_end_offset, 'eeeeeeeeeeeeeeeeeeeeee!')
